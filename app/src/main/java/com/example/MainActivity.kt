    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            MyApplicationTheme {
                ...
                val boardViewModel: com.example.presentation.viewmodel.BoardViewModel = viewModel(
                    factory = com.example.presentation.viewmodel.BoardViewModel.provideFactory(application)
                )
                ...
                PushRevealDrawer(
                    ...
                    Scaffold(
                        ...
                        composable("sermons") {
                            SermonOrganizerScreen(
                                viewModel = appViewModel,
                                onBack = { navController.popBackStack() },
                                onNavigateToCalendar = { navController.navigate("sermon_calendar") },
                                onNavigate = { route ->
                                    navController.navigate(route) {
                                        popUpTo("home") { saveState = true }
                                        launchSingleTop = true
                        ...
                        composable("sermon_viewer") {
                            SermonViewerScreen(
                                ...
                                onNavigateToPreach = { id, file, title, duration, speed, scale ->
                                    ...
                                    appViewModel.livePreachScrollSpeed = speed
                                    appViewModel.livePreachFontScale = scale
                                    navController.navigate("live_preaching")
                                },
                                onNavigateToAiPreach = { id, file, title, duration ->
                                    appViewModel.activeViewerSermonId = id
                                    appViewModel.activeViewerFilePath = file
                                    appViewModel.activeViewerTitle = title
                                    appViewModel.livePreachDurationMinutes = duration
                                    navController.navigate("ai_preach_mode")
                                }
                            )
                        }

                        composable("live_preaching") {
                            LivePreachingScreen(
                                viewModel = appViewModel,
                                sermonId = appViewModel.activeViewerSermonId,
                                filePath = appViewModel.activeViewerFilePath,
                                ...
                                scrollSpeed = appViewModel.livePreachScrollSpeed,
                                fontScale = appViewModel.livePreachFontScale,
                                onBack = { navController.popBackStack() },
                                onNavigateToAltarCall = { navController.navigate("altar_call") }
                            )
                        }

                        composable("altar_call") {
                            AltarCallScreen(
                                onBack = { navController.popBackStack() }
                            )
                        }

                        composable("ai_preach_mode") {
                            PreachModeScreen(
                                filePath = appViewModel.activeViewerFilePath,
                                sermonTitle = appViewModel.activeViewerTitle,
                                durationMinutes = appViewModel.livePreachDurationMinutes,
                        ...
                        composable("board_library") {
                            com.example.presentation.screens.BoardLibraryScreen(
                                viewModel = boardViewModel,
                                onBack = { navController.popBackStack() },
                                onNavigateToBoard = { boardId ->
                                    boardViewModel.selectBoard(boardId)
                                    navController.navigate("board_canvas")
                                },
                                onNavigate = { route ->
                                    navController.navigate(route) {
                                        popUpTo("home") { saveState = true }
                        ...
                        composable("board_canvas") {
                            com.example.presentation.screens.BoardCanvasScreen(
                                viewModel = boardViewModel,
                                onBack = { navController.popBackStack() }
                            )
                        }
